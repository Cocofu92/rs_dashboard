...
def get_pct_change_and_emas(ticker):
    ...
    try:
        ...
        ema_100 = pd.Series(closes).ewm(span=100).mean().iloc[-1]
...
with st.spinner("Fetching data and calculating relative strength..."):
    ...
    if not benchmark_data:
        st.error("❌ Benchmark data (SPY) fetch failed or has <100 candles.")
    else:
        st.info(f"✅ SPY Benchmark Debug: Price={benchmark_data['price']:.2f}, "
                f"EMA50={benchmark_data['ema_50']:.2f}, EMA100={benchmark_data['ema_100']:.2f}, "
